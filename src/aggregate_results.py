"""
KROK 3 — Agregacja wyników: z results.csv robi czytelne tabele i wykresy.

To gotowe materiały do raportu eksperymentów (dla Experiments Lead)
oraz do slajdów (dla Presentation Lead).
...
def main():
    ...
    df = df.sort_values(["dataset", "weights", "imgsz"]).reset_index(drop=True)

    # tabela zbiorcza
    summary = df[[
        "weights", "imgsz", "params_M", "GFLOPs",
    ...
    # wykres 1: kompromis mAP vs czas (każdy punkt = jeden model)
    plt.figure(figsize=(8, 6))
    for _, r in df.iterrows():
        plt.scatter(r["ms_per_image"], r["mAP50_95"], s=80)
        plt.annotate(f"{r['weights']}\n@{int(r['imgsz'])}",
    ...
    plt.title("Kompromis dokładność ↔ szybkość")
    ...
    # wykres 2: słupki mAP per model
    plt.figure(figsize=(9, 5))
    labels = df["weights"] + " @" + df["imgsz"].astype(str)
    plt.bar(labels, df["mAP50_95"])
    plt.ylabel("mAP@50-95")
    plt.title("mAP@50-95 dla porównywanych modeli")
